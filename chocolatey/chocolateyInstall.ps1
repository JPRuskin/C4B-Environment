...
if ($PackageInternalized = Test-Path "$PSScriptRoot\files") {
    # This section ensures the package internalizes successfully for airgapped environments, but is not required for the module to work otherwise.
    $InternalizedZipArguments = @{
        PackageName   = "c4b-environment.powershell"
        Url           = "https://www.nuget.org/api/v2/package/BCrypt.Net/0.1.0"
        Checksum      = "E3D2A5BCF2AA9768279001020859D7F3F0AB207002DA67ED608C33201B5B5600"
        ChecksumType  = "sha256"
        UnzipLocation = "$PSScriptRoot\module\data\bcrypt.net.0.1.0"
    }
    Install-ChocolateyZipPackage @InternalizedZipArguments
}
...
if ($PSVersionTable.PSVersion.Major -lt 4 -or -not (Get-Module "C4B-Environment" -ListAvailable)) {
